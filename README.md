...
### 数据库初始化
```bash
mysql -u root -p < sql/xixin_health_clean.sql
mysql -u root -p xixin_health < sql/xixin_health_demo_data.sql
```
...
├── frontend/               # Vue 3 SPA
│   ├── src/
│   │   ├── views/          # 35 个页面（guest/user/doctor/operator/admin）
...
深色主题设计系统，定义在 `frontend/src/styles/index.scss`：
...
| [复杂业务设计](复杂业务设计文档.md) | 智能排班、多室路由、多年对比等 |
